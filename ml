...
<form id="signup-student-form" class="auth-form" onsubmit="handleSignUp(event, 'student')" style="display: none;">
    ...
    <h3 style="color: #0d3b66; margin-bottom: 15px; text-align: center;">🎓 Student Sign Up</h3>
    ...
    <div id="signup-student-error" class="feedback error" style="display:none; margin-top:15px;"></div>
            ...
            <form id="signup-coach-form" class="auth-form" onsubmit="handleSignUp(event, 'coach')" style="display: none;">
                ...
                <h3 style="color: #0d3b66; margin-bottom: 15px; text-align: center;">👨‍🏫 Coach Sign Up</h3>
            ...
            <form id="signup-organization-form" class="auth-form" onsubmit="handleSignUp(event, 'organization')" style="display: none;">
                ...
                <h3 style="color: #0d3b66; margin-bottom: 15px; text-align: center;">🏫 Organization Sign Up</h3>
    ...
    <button class="quiz-tab" onclick="switchSpellingMode('unscramble')">Unscramble</button>
        ...
        <div id="spelling-game" style="display: none;">
            <div class="quiz-question-card">
                ...
                <div class="virtual-keyboard">
                    ...
                    <div class="keyboard-row">
                        <button class="keyboard-key backspace" onclick="backspaceKey()">⌫ Delete</button>
                        <button class="keyboard-key space" onclick="typeKey(' ')">Space</button>
                        <button class="keyboard-key enter" onclick="submitSpelling()">✓ Enter</button>
                    </div>
                </div>
...
<div id="spelling-incorrect-section" class="content-section">
    ...
    <!-- Unscramble Mode -->
<div id="unscramble-spelling-mode" class="quiz-mode">
    <div id="unscramble-container">
        <div class="welcome-section">
            <h2>🔀 Word Unscramble</h2>
            <p>Unscramble the letters to form the correct word. Test your word recognition skills!</p>
            
            <div style="margin: 20px 0;">
                <label style="display: block; margin-bottom: 10px; font-weight: bold;">Difficulty Level:</label>
                <select id="unscramble-difficulty" style="padding: 10px; font-size: 1rem; border: 2px solid #0d3b66; border-radius: 5px; width: 200px;">
                    <option value="easy">Easy (3-5 letters)</option>
                    <option value="medium" selected>Medium (6-8 letters)</option>
                    <option value="hard">Hard (9+ letters)</option>
                </select>
            ...
            <button class="practice-btn" onclick="startUnscramblePractice()">Start Unscramble Practice</button>
        </div>
        ...
        <div id="unscramble-game" style="display: none;">
            <div class="quiz-question-card">
                ...
                <div style="text-align: center; margin: 30px 0;">
                    <h2 style="color: #0d3b66; margin-bottom: 20px;">Unscramble these letters:</h2>
                    <div id="scrambled-letters" style="font-size: 2.5rem; font-weight: bold; color: #0d3b66; letter-spacing: 10px; margin: 30px 0; padding: 20px; background: #e3f2fd; border-radius: 10px; font-family: monospace;">
                        LOADING
                    </div>
                    
                    <div style="display: flex; gap: 8px; justify-content: center; flex-wrap: wrap; margin: 20px 0;">
    <button class="practice-btn" onclick="giveUnscrambleHint()" style="font-size: 0.9rem; padding: 10px 15px; background: #ff9800;">
        💡 Get Hint
    </button>
    <button class="practice-btn" id="show-unscramble-details-btn" onclick="showUnscrambleWordDetails()" style="font-size: 0.9rem; padding: 10px 15px; display: none;">
        📖 Word Details
    </button>
</div>
                </div>
               ...
               <div class="form-group" style="margin: 30px 0;">
    <label style="font-size: 1.2rem;">Type your answer:</label>
    <input type="text" id="unscramble-input" 
           placeholder="Type the unscrambled word..." 
           style="width: 100%; padding: 15px; font-size: 1.3rem; text-align: center; border: 3px solid #0d3b66; border-radius: 8px; margin-top: 10px;"
           onkeypress="if(event.key === 'Enter') submitUnscramble()">
</div>

<div style="display: flex; gap: 10px; justify-content: center; margin-top: 20px; flex-wrap: wrap;">
    <button class="practice-btn" onclick="shuffleScrambledWord()" style="background: #ff9800; font-size: 1.1rem;">
        🔀 Shuffle Letters
    </button>
    <button class="practice-btn" onclick="submitUnscramble()" style="background: #28a745; font-size: 1.1rem;">
        ✓ Submit Answer
    </button>
</div>
                            
                <div style="text-align: center; margin-top: 20px;">
                    <button class="practice-btn" onclick="skipUnscrambleWord()" style="background: #666;">
                        ⏭️ Skip
                    </button>
                </div>
                            
                <div id="unscramble-feedback" style="margin-top: 20px;"></div>
                <div id="unscramble-hint" style="margin-top: 15px; padding: 12px; background: #fff3cd; border-left: 3px solid #ffc107; border-radius: 5px; display: none;"></div>
            </div>
        </div>
        ...
        <div class="dashboard-section">
            ...
            <div class="dashboard-grid">
                ...
                <div class="dashboard-card dashboard-card-clickable" onclick="navigateTo('incorrect-quiz')">
                    ...
                    <div class="card-action">Practice →</div>
                </div>
...
let unscrambleMistakes = [];
let hintsUsed = 0;



        // ===== ADMIN CHECK =====
const ADMIN_EMAIL = 'wordbizcoachingacademy@gmail.com';
...
auth.onAuthStateChanged(async user => {
    ...
    // ADD THIS: Load dashboard when navigating to it
...
async function displayWords(words) {
    ...
    wordList.innerHTML = words.map(word => `
        <div class="word-card" onclick="showWordDetail('${word}')">
            ...
            <div class="word-definition" id="phonetic-${word}">Loading pronunciation...</div>
        </div>
    `).join('');
    
    // Fetch phonetics for each word
    words.forEach(async (word) => {
        const phoneticEl = document.getElementById(`phonetic-${word}`);
        ...
        try {
            ...
            if (wordDoc.exists && wordDoc.data().phonetic) {
                phoneticEl.textContent = wordDoc.data().phonetic;
            } else {
                ...
                if (response.ok) {
                    ...
                    if (data.result && data.result.phonetic) {
                        phoneticEl.textContent = data.result.phonetic;
                    } else {
                        phoneticEl.textContent = 'Click to see full definition';
                    }
                } else {
                    phoneticEl.textContent = 'Click to see full definition';
                }
    ...
    if (!cachedData.origin) {
        console.log('⚠️ Origin missing - fetching from API...');
        try {
            const response = await fetch(CLOUD_FUNCTION_URL, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (response.ok) {
                ...
                if (data.result && data.result.origin) {
                    ...
                    console.log('✅ Origin fetched and saved!');
                }
            }
        } catch (error) {
...
function displayWordDetails(wordData) {
    ...
    if (wordData.origin) {
        html += `
            <div style="margin-bottom: 25px; padding: 15px; background: #fff3cd; border-left: 4px solid #ffc107; border-radius: 5px;">
                <strong style="color: #856404; font-size: 1.1rem;">📜 Origin:</strong>
                <p style="color: #856404; margin-top: 8px; line-height: 1.6;">${wordData.origin}</p>
            </div>
        `;
    }
...
// ⭐ NEW HELPER FUNCTION - Fetch word details with origin
...
async function handleSignUp(event, userType) {
    ...
    let additionalData = { userType: userType };
    if (userType === 'student') {
...
async function signUpWithGoogle(userType) {
    ...
    try {
        ...
        closeAuthModal();
        alert(`Welcome, ${user.displayName}! Your ${userType} account has been created successfully.`);
    } catch (error) {
...
function getIQCategory(iq) {
    if (iq >= 130) return { label: "Genius", color: "#FFD700", emoji: "🧠✨" };
    if (iq >= 120) return { label: "Superior", color: "#4CAF50", emoji: "🌟" };
    if (iq >= 110) return { label: "High Average", color: "#2196F3", emoji: "⭐" };
    if (iq >= 90) return { label: "Average", color: "#FF9800", emoji: "👍" };
    if (iq >= 80) return { label: "Low Average", color: "#FF5722", emoji: "📚" };
    return { label: "Keep Practicing", color: "#9E9E9E", emoji: "💪" };
}
    ...
    container.innerHTML = `
        <div class="quiz-question-card" style="text-align: center;">
            ...
            <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 20px; margin: 30px 0; box-shadow: 0 10px 30px rgba(0,0,0,0.2);">
                <h3 style="color: white; margin-bottom: 15px; font-size: 1.5rem;">🧠 Your Vocabulary IQ</h3>
                ...
                <div style="background: rgba(255,255,255,0.2); padding: 15px; border-radius: 10px; margin-top: 20px;">
                    <p style="color: white; margin: 8px 0;"><strong>📊 Accuracy:</strong> ${iqData.accuracy.toFixed(1)}%</p>
                    <p style="color: white; margin: 8px 0;"><strong>⚡ Avg Response Time:</strong> ${iqData.avgTime}s</p>
                    <p style="color: white; margin: 8px 0;"><strong>🎯 Speed Score:</strong> ${iqData.speedScore}/100</p>
                </div>
            </div>
            ...
            ${showSpellingEncouragement ? `
                <div style="background: linear-gradient(135deg, #FFD700 0%, #FFA500 100%); padding: 25px; border-radius: 15px; margin: 20px 0; box-shadow: 0 5px 20px rgba(255,215,0,0.4); animation: pulse-glow 2s infinite;">
                    <h3 style="color: #0d3b66; margin-bottom: 10px;">🏆 You Have Spelling Champion Potential!</h3>
                    ...
                    <p style="color: #0d3b66; margin: 10px 0; font-weight: bold;">
                        💰 Head to the Spelling section to challenge users in live challenges and earn up to 5 coins per question!
                    </p>
...
function displayFavorites() {
    ...
    feedbackDiv.innerHTML = '<div class="loading">📚 Loading word details...</div>';
    ...
    try {
        ...
        feedbackDiv.innerHTML = `
            <div style="background: white; padding: 20px; border-radius: 8px; border: 2px solid #17a2b8; margin-top: 15px;">
                <h3 style="color: #17a2b8; margin-bottom: 15px;">📖 Word Information</h3>
                
                ${wordDetails.phonetic ? `
                    <div style="margin-bottom: 15px;">
                        <strong style="color: #0d3b66;">🔊 Pronunciation:</strong>
                        <span style="font-style: italic; color: #666; margin-left: 8px;">${wordDetails.phonetic}</span>
                    </div>
                ` : ''}
                
                ${wordDetails.origin ? `
                    <div style="margin-bottom: 15px; padding: 12px; background: #fff3cd; border-left: 3px solid #ffc107; border-radius: 5px;">
                        <strong style="color: #856404;">📜 Origin:</strong>
                        <p style="color: #856404; margin-top: 8px; line-height: 1.6; font-size: 0.95rem;">${wordDetails.origin}</p>
                    </div>
                ` : ''}
                
                ${wordDetails.definition ? `
                    <div style="margin-bottom: 10px;">
                        <strong style="color: #0d3b66;">📝 Definition:</strong>
                ...
                <p style="margin-top: 15px; padding: 10px; background: #e3f2fd; border-radius: 5px; color: #0d3b66; font-style: italic; text-align: center;">
                    ⚠️ Remember: Don't use this to cheat! Use it to learn about the word after attempting.
                </p>
            </div>
        `;
    } catch (error) {
    ...
    if (userAnswer === correctWord) {
        ...
        feedbackDiv.innerHTML = '<div class="loading">✅ Correct! Loading word details...</div>';
        ...
        try {
            ...
            feedbackDiv.innerHTML = `
                <div class="feedback success">
                    ✅ <strong>Correct!</strong> The word is: <strong>${currentSpellingWord}</strong>
                    ...
                    ${wordDetails.origin ? `
                        <div style="margin-top: 15px; padding: 12px; background: #fff3cd; border-left: 3px solid #ffc107; border-radius: 5px;">
                            <strong style="color: #856404;">📜 Origin:</strong>
                            <p style="color: #856404; margin-top: 5px; font-size: 0.95rem;">${wordDetails.origin}</p>
                        </div>
                    ` : ''}
                    
                    ${wordDetails.definition ? `
                        <div style="margin-top: 10px; color: #0d3b66;">
        ...
        // Fetch word details to show origin
        feedbackDiv.innerHTML = '<div class="loading">❌ Incorrect. Loading word details...</div>';
        ...
        try {
            ...
            feedbackDiv.innerHTML = `
                <div class="feedback error">
                    ❌ <strong>Incorrect.</strong> You wrote: <strong>${userAnswer}</strong><br>
                    Correct spelling: <strong>${currentSpellingWord}</strong>
                    
                    ${currentUser ? '<p style="margin-top: 10px; color: #721c24; font-style: italic;">✅ Automatically saved to Spelling Mistakes</p>' : ''}
    ...
    const feedbackDiv = document.getElementById('spelling-feedback');
    feedbackDiv.innerHTML = '<div class="loading">⏭️ Loading word details...</div>';
    ...
    try {
        ...
        feedbackDiv.innerHTML = `
            <div class="feedback" style="background: #fff3cd; border-color: #ffc107; color: #856404;">
                ⏭️ <strong>Skipped.</strong> The word was: <strong>${currentSpellingWord}</strong>
                ...
                ${wordDetails.origin ? `
                    <div style="margin-top: 15px; padding: 12px; background: #ffe8a1; border-left: 3px solid #ff9800; border-radius: 5px;">
                        <strong>📜 Origin:</strong>
                        <p style="margin-top: 5px; font-size: 0.95rem;">${wordDetails.origin}</p>
                    </div>
                ` : ''}
    ...
    // Hide ALL spelling modes first
    document.getElementById('practice-spelling-mode').classList.remove('active');
    document.getElementById('unscramble-spelling-mode').classList.remove('active');
    document.getElementById('live-spelling-mode').classList.remove('active');
    document.getElementById('leaderboard-spelling-mode').classList.remove('active');

    // Then show only the selected mode
    if (mode === 'practice') {
        document.getElementById('practice-spelling-mode').classList.add('active');
    } else if (mode === 'unscramble') {
        document.getElementById('unscramble-spelling-mode').classList.add('active');
    } else if (mode === 'live') {
    ...
    container.innerHTML = `
        ...
        <div class="favorites-grid">
            ${userSpellingFavorites.map(fav => `
                <div class="favorite-card">
                    <button class="delete-btn" onclick="removeSpellingFavorite('${fav.id}')">×</button>
                    <h3 style="color: #0d3b66; margin-bottom: 10px;">📝 ${fav.word}</h3>
                    ...
                    <p style="color: ${fav.wasCorrect ? '#28a745' : '#dc3545'}; margin-top: 10px; font-weight: bold;">
                        ${fav.wasCorrect ? '✅ Spelled Correctly' : '❌ Needs Practice'}
                    </p>
                </div>
            `).join('')}
        </div>
    ...
    container.innerHTML = `
        ...
        <div class="favorites-grid">
            ${userSpellingIncorrect.map((inc, index) => `
                <div class="favorite-card">
                    <button class="delete-btn" onclick="removeSpellingIncorrect('${inc.id || index}')">×</button>
                    <h3 style="color: #0d3b66; margin-bottom: 10px;">📝 ${inc.word}</h3>
...
async function loadUserDashboard() {
    ...
    try {
        ...
        // Format join date - Add edit button ONLY if current user is admin
        if (userData?.createdAt) {
            const joinDate = userData.createdAt.toDate();
            const formattedDate = joinDate.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
            
            // Only show edit button if the logged-in user is the admin
            if (isAdmin()) {
                document.getElementById('dashboard-join-date').innerHTML = 
                    `Member since: ${formattedDate} <button onclick="editMyJoinDate()" style="background: #ffd700; color: #0d3b66; border: none; padding: 5px 10px; border-radius: 5px; cursor: pointer; margin-left: 10px; font-size: 0.8rem;">✏️ Edit</button>`;
            } else {
                document.getElementById('dashboard-join-date').textContent = `Member since: ${formattedDate}`;
            }
            
            document.getElementById('display-joined').textContent = formattedDate;
        } else {
            // Fallback for accounts without createdAt
            const fallbackText = 'Recently joined';
            if (isAdmin()) {
                document.getElementById('dashboard-join-date').innerHTML = 
                    `Member since: ${fallbackText} <button onclick="editMyJoinDate()" style="background: #ffd700; color: #0d3b66; border: none; padding: 5px 10px; border-radius: 5px; cursor: pointer; margin-left: 10px; font-size: 0.8rem;">✏️ Edit</button>`;
            } else {
                document.getElementById('dashboard-join-date').textContent = `Member since: ${fallbackText}`;
            }
            document.getElementById('display-joined').textContent = fallbackText;
        }
        ...
        async function editMyJoinDate() {
    if (!isAdmin()) {
        alert('You do not have permission to edit the join date.');
        return;
    }
    
    const currentDate = await getMyCurrentJoinDate();
    const newDateStr = prompt('Enter new join date (YYYY-MM-DD format):\nExample: 2024-01-15', currentDate);
    
    if (!newDateStr) return; // User cancelled
    ...
    if (confirm(`Change your join date to ${newDate.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}?`)) {
        await updateMyJoinDate(newDate);
    }
}

async function getMyCurrentJoinDate() {
    try {
        const userDoc = await db.collection('users').doc(currentUser.uid).get();
        const userData = userDoc.data();
...
async function updateMyJoinDate(newDate) {
    try {
        ...
        alert('✅ Your join date has been updated successfully!');
        await loadUserDashboard(); // Reload dashboard to show new date
    } catch (error) {
        console.error('Error updating join date:', error);
        alert('❌ Failed to update join date: ' + error.message);
    }
}
...
function backspaceKey() {
    ...
    input.value = input.value.slice(0, -1);
        ...
        // ===== UNSCRAMBLE PRACTICE FUNCTIONS =====
function startUnscramblePractice() {
    const difficulty = document.getElementById('unscramble-difficulty').value;
    ...
    document.querySelector('#unscramble-container .welcome-section').style.display = 'none';
...
function loadUnscrambleWord() {
    ...
    currentScrambledWord = scrambleWord(currentUnscrambleWord);
    ...
    document.getElementById('unscramble-hint').style.display = 'none';
    document.getElementById('unscramble-hint').innerHTML = '';
    
    document.getElementById('scrambled-letters').textContent = currentScrambledWord.toUpperCase();
    
    const detailsBtn = document.getElementById('show-unscramble-details-btn');
    if (detailsBtn) {
        detailsBtn.style.display = 'none';
    }
    
    document.getElementById('unscramble-input').disabled = false;
    document.getElementById('unscramble-input').focus();
}
...
function speakUnscrambleWord() {
    if (!currentUnscrambleWord) return;
    
    const utterance = new SpeechSynthesisUtterance(currentUnscrambleWord);
    utterance.rate = 0.7;
    utterance.pitch = 1;
    utterance.volume = 1;
...
function giveUnscrambleHint() {
    const hintDiv = document.getElementById('unscramble-hint');
    hintsUsed++;
    
    const hintLevel = Math.min(hintsUsed, 3);
    
    if (hintLevel === 1) {
        hintDiv.innerHTML = `<strong>💡 Hint 1:</strong> The word starts with "<strong>${currentUnscrambleWord.charAt(0).toUpperCase()}</strong>"`;
    } else if (hintLevel === 2) {
        hintDiv.innerHTML = `<strong>💡 Hint 2:</strong> The word is <strong>${currentUnscrambleWord.length}</strong> letters long and starts with "<strong>${currentUnscrambleWord.charAt(0).toUpperCase()}</strong>"`;
    } else {
        const revealed = currentUnscrambleWord.substring(0, Math.ceil(currentUnscrambleWord.length / 2));
        const hidden = '_'.repeat(currentUnscrambleWord.length - revealed.length);
        hintDiv.innerHTML = `<strong>💡 Hint 3:</strong> The word is: <strong style="font-family: monospace; font-size: 1.2rem;">${revealed.toUpperCase()}${hidden}</strong>`;
    }
    
    hintDiv.style.display = 'block';
}

async function showUnscrambleWordDetails() {
    if (!currentUnscrambleWord) return;
...
async function submitUnscramble() {
    ...
    if (userAnswer === correctWord) {
        ...
        try {
            ...
            feedbackDiv.innerHTML = `
                <div class="feedback success">
                    ✅ <strong>Correct!</strong> The word is: <strong>${currentUnscrambleWord}</strong>
            ...
            feedbackDiv.innerHTML = `
                <div class="feedback success">
                    ✅ Correct! The word is: <strong>${currentUnscrambleWord}</strong>
        ...
        unscrambleMistakes.push({
            word: currentUnscrambleWord,
            userAnswer: userAnswer,
            scrambled: currentScrambledWord
        });
        ...
        try {
            ...
            feedbackDiv.innerHTML = `
                <div class="feedback error">
                    ❌ <strong>Incorrect.</strong> You wrote: <strong>${userAnswer}</strong><br>
                    Correct word: <strong>${currentUnscrambleWord}</strong>
        ...
        setTimeout(() => speakUnscrambleWord(), 1000);
    }
    
    document.getElementById('show-unscramble-details-btn').style.display = 'inline-block';
}

async function skipUnscrambleWord() {
    unscrambleMistakes.push({
        word: currentUnscrambleWord,
        userAnswer: '(skipped)',
        scrambled: currentScrambledWord
    ...
    try {
        ...
        feedbackDiv.innerHTML = `
            <div class="feedback" style="background: #fff3cd; border-color: #ffc107; color: #856404;">
                ⏭️ <strong>Skipped.</strong> The word was: <strong>${currentUnscrambleWord}</strong>
        ...
        feedbackDiv.innerHTML = `
            <div class="feedback" style="background: #fff3cd; border-color: #ffc107; color: #856404;">
                ⏭️ Skipped. The word was: <strong>${currentUnscrambleWord}</strong>
...
function showUnscrambleResults() {
    ...
    if (percentage >= 90) {
        message = 'Outstanding! You\'re an unscramble master!';
        emoji = '🏆';
    } else if (percentage >= 70) {
        message = 'Great job! Keep practicing!';
        emoji = '⭐';
    ...
    resultsDiv.innerHTML = `
        <div class="quiz-question-card" style="text-align: center;">
            <div style="font-size: 4rem; margin: 20px 0;">${emoji}</div>
            <h2>Unscramble Practice Complete!</h2>
            <div class="quiz-score">Final Score: ${unscrambleScore} / ${unscrambleWords.length}</div>
            <p style="font-size: 1.5rem; margin: 20px 0; color: #0d3b66;">${percentage}%</p>
            <p style="font-size: 1.2rem; margin: 20px 0;">${message}</p>
            
            ${unscrambleMistakes.length > 0 ? `
                <div style="margin: 30px 0; text-align: left; max-width: 600px; margin-left: auto; margin-right: auto;">
                    <h3 style="color: #0d3b66; margin-bottom: 15px;">📝 Words to Review:</h3>
                    <div style="background: #f9f9f9; padding: 20px; border-radius: 8px;">
                        ${unscrambleMistakes.map(mistake => `
                            <div style="padding: 10px; margin: 5px 0; background: white; border-left: 3px solid #dc3545; border-radius: 5px;">
                                <strong>${mistake.word}</strong> 
                                <span style="color: #666;">- Scrambled: ${mistake.scrambled}</span><br>
                                <span style="color: #999; font-size: 0.9rem;">Your answer: ${mistake.userAnswer}</span>
                            </div>
                        `).join('')}
                    </div>
                </div>
            ...
            <div style="display: flex; gap: 10px; justify-content: center; margin-top: 30px; flex-wrap: wrap;">
                <button class="practice-btn" onclick="startUnscramblePractice()">🔄 Practice Again</button>
                <button class="practice-btn" onclick="restartUnscrambleSection()">📚 Change Difficulty</button>
                <button class="practice-btn" onclick="navigateTo('home')">🏠 Home</button>
            </div>
        </div>
    `;
...
function restartUnscrambleSection() {
    document.getElementById('unscramble-game').style.display = 'none';
    document.getElementById('unscramble-results').style.display = 'none';
    document.querySelector('#unscramble-container .welcome-section').style.display = 'block';
}

function typeUnscrambleKey(letter) {
    const input = document.getElementById('unscramble-input');
    if (input.disabled) return;
    input.value += letter.toLowerCase();
}

function backspaceUnscrambleKey() {
    const input = document.getElementById('unscramble-input');
    if (input.disabled) return;
    input.value = input.value.slice(0, -1);
}

function clearUnscrambleInput() {
    const input = document.getElementById('unscramble-input');
    if (input.disabled) return;
    input.value = '';
}

        function shuffleScrambledWord() {
    let newScramble;
    
    // Keep shuffling until we get a different arrangement (can be the correct word or another scramble)
    do {
        newScramble = currentUnscrambleWord.split('').sort(() => Math.random() - 0.5).join('');
    } while (newScramble === currentScrambledWord);
    
    currentScrambledWord = newScramble;
    document.getElementById('scrambled-letters').textContent = currentScrambledWord.toUpperCase();
    
    // Visual feedback
    const letterDiv = document.getElementById('scrambled-letters');
    letterDiv.style.transform = 'scale(1.1) rotate(5deg)';
    setTimeout(() => letterDiv.style.transform = 'scale(1) rotate(0deg)', 300);
}
    </script>
</body>
</html>
